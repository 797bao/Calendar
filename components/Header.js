...
import { View, Text, Platform, Image, TouchableOpacity,SafeAreaView } from 'react-native'
...
const Header = ({header_color, left_icon, accent, status_bar}) => {
  ...
  return (
    <AppContext.Consumer>
      {(context) => {
        return (
          <SafeAreaView style={[styles.container, {backgroundColor: header_color}]} >
            <View style={styles.text_row}>
              { left_icon }
              <TouchableOpacity style={styles.center} onPress={() => context.toggleDatePicker()} /* the entire center*/>
                <View style={styles.day_box}>
                  <Text style={[styles.dotw, {color: accent_color}]}>{moment(context.date).format("ddd").toUpperCase() /*M-S */}</Text>
                  <View style={[styles.circle, {backgroundColor: accent_color}]}>
                    <Text style={[styles.day, {color: "#000"}]}>{moment(context.date).format("D") /* day # */}</Text>
                  </View>
                </View>
              ...
              <TouchableOpacity style={styles.img} onPress={() => context.goToToday()} /*calender button to go to TODAY*/>  
                <Image source={icon}/>
              </TouchableOpacity>
            </View>
          </SafeAreaView>
